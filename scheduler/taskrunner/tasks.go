...
func deleteVideo_Local(vid string) error {
    err := os.Remove(VIDEO_PATH + vid)

    if err != nil && !os.IsNotExist(err) {
        log.Printf("Deleting video error, oss operation faile _local")
        return errors.New("Deleting video error")
    }
    return nil
}

func VideoClearDispatcher_Local(dc dataChan) error {
    res, err := dbops.ReadVideoDeletionRecord(5000)
    if err != nil {
        log.Printf("Video clear dispatcher error: %v", err)
        return err
...
func VideoClearExecutor_Local(dc dataChan) error {
    errMap := &sync.Map{}
    var err error
...
forloop:
    for {
        ...
        case vid := <-dc:
            go func(id interface{}) {
                if err := deleteVideo_Local(id.(string)); err != nil {
                    errMap.Store(id, err)
                    return
                }
                if err := dbops.DelVideoDeletionRecord(id.(string)); err != nil {
